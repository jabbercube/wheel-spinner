---
name: validate-system-review
description: Analyze implementation against plan for process improvements. Meta-level review of how well implementation followed the plan.
---
...
Execution report: $ARGUMENTS[1]

## Analysis Workflow

### Step 1: Understand the Planned Approach

Read the generated plan ($ARGUMENTS[0]) and extract:
...
Read the execution report ($ARGUMENTS[1]) and extract:
...
- Plan reviewed: [path to $ARGUMENTS[0]]
- Execution report: [path to $ARGUMENTS[1]]
- Date: [current date]
...
- [ ] `/[skill-name]` for [manual process repeated 3+ times]
